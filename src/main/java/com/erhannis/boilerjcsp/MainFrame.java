...
import org.jcsp.lang.Any2AnyCallChannel;
...
public class MainFrame extends javax.swing.JFrame {
  ...
  private void initComponents() {
    ...
    btnExampleCallProcess = new javax.swing.JButton();
    ...
    btnExampleCallProcess.setText("CP");
    btnExampleCallProcess.setToolTipText("Example: channel");
    btnExampleCallProcess.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(java.awt.event.ActionEvent evt) {
        btnExampleCallProcessActionPerformed(evt);
      }
    });
    ...
    jPanel1Layout.setHorizontalGroup(
      ...
      .addGroup(jPanel1Layout.createSequentialGroup()
        ...
        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
        .addComponent(btnExampleCallProcess)
        .addContainerGap())
  ...
  private void btnExampleCallProcessActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExampleCallProcessActionPerformed
    taSpecification.setText("name: Server\n"
            + "type: callprocess\n"
            + "mode: any2one\n"
            + "method: getInt int\n"
            + "method: setInt void int value\n"
            + "method: processString java.lang.String java.lang.String arg1 int arg2\n"
            + "method: sync void");
  }//GEN-LAST:event_btnExampleCallProcessActionPerformed
  ...
  private static String generate(String spec) throws IOException, JClassAlreadyExistsException {
    ...
    switch (type) {
      ...
      case "callprocess":
        return makeCallProcess(spec, name, lines);
      default:
  ...
  private static String makeCallProcess(String spec, String name, List<String> lines) throws JClassAlreadyExistsException, IOException {
//        taSpecification.setText("name: Server\n" +
//                                "type: callprocess\n" +
//                                "mode: any2one\n" +
//                                "method: getInt int\n" +
//                                "method: setInt void int\n" +
//                                "method: processString java.lang.String java.lang.String int\n" +
//                                "method: sync void");
    String mode;
    String clazz;
    String line;
    ...
    ArrayList<MethodDescription> methods = new ArrayList<>();
    for (String l : lines) {
      if (l.startsWith("method:")) {
        l = l.substring("method:".length(), l.length());
        String[] tokens = l.trim().split(" +");
        String methodName = tokens[0];

        String returnType = tokens[1];
        String[] argTypes = Arrays.copyOfRange(tokens, 2, tokens.length - 1);
        asdf;
        methods.add(new MethodDescription(methodName, returnType, argTypes));
        throw new IllegalArgumentException("Line 3+ must start with \"method\"");
      }
    }
    ...
    p.javadoc().add(""
            ...
            + "Specification:\n"
            + spec);
    JDefinedClass cc = p._class(JMod.PRIVATE | JMod.STATIC, name + "CallChannel");
    switch (mode) {
      case "o2o":
      case "one2one":
        cc._extends(One2OneCallChannel.class);
        break;
      case "o2a":
      case "one2any":
        cc._extends(One2AnyCallChannel.class);
        break;
      case "a2o":
      case "any2one":
        cc._extends(Any2OneCallChannel.class);
        break;
      case "a2a":
      case "any2any":
        cc._extends(Any2AnyCallChannel.class);
        break;
      default:
        throw new IllegalArgumentException("Mode must be one of: {o2o, o2a, a2o, a2a, one2one, one2any, any2one, any2any}");
    }
    { // Call Channel
      ArrayList<JFieldVar> methodConsts = new ArrayList<>();
      for (int i = 0; i < methods.size(); i++) {
        MethodDescription method = methods.get(i);
        JFieldVar mConst = cc.field(JMod.PRIVATE | JMod.STATIC | JMod.FINAL, int.class, "METHOD_" + method.name, JExpr.lit(i));
        methodConsts.add(mConst);
      }
      for (int i = 0; i < methodConsts.size(); i++) {
        MethodDescription mDesc = methods.get(i);
        JFieldVar mConst = methodConsts.get(i);
        AbstractJType returnType;
        JMethod cc_m;
        boolean returnsVoid;
        if ("void".equals(mDesc.returnType)) {
          returnsVoid = true;
          cc_m = cc.method(JMod.PRIVATE, Void.TYPE, mDesc.name);
        } else {
          returnsVoid = false;
          cc_m = cc.method(JMod.PRIVATE, codeModel.ref(mDesc.returnType), mDesc.name);
        }
        ArrayList<JVar> args = new ArrayList<>();
        for (int j = 0; j < mDesc.argTypes.length; j++) {
          JVar cc_m_val = cc_m.param(codeModel.ref(mDesc.argTypes[j]), mDesc.argNames[j]);
        }
        cc_m.body().add(mico.invoke("write").arg(JExpr._new(cc).arg(mConst).arg(JExpr.lit(0)).arg(cc_m_val)));//TODO wrong
      }
  ...
  private javax.swing.JButton btnExampleCallProcess;
